...
const Hero = () => {
  ...
  return (
    <section className="relative flex min-h-[90vh] flex-col items-center justify-center overflow-hidden px-4 py-20 text-center md:px-6 lg:min-h-screen mt-8">
      {/* Fondo con gradiente */}
      <Gradient className="h-60 w-60 left-40"/>

      {/* Etiqueta superior */}
      <div className="relative z-10 mb-4 flex items-center gap-2">
        <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-secondary" />
        <span className="rounded-full bg-secondary px-3 py-[2px] text-[9px] font-medium tracking-wider text-white/80 md:px-4 md:text-[10px]">
      ...
      <h1 className={`${fontTitulo.className} relative z-10 max-w-5xl uppercase tracking-tighter text-foreground leading-[0.9]`}>
        <span className="mb-2 block text-[42px] sm:text-7xl md:text-8xl lg:text-9xl">
          Tu negocio en
        </span>
        <span className="flex flex-col items-center justify-center gap-2 sm:flex-row sm:gap-4 my-2">
          <span className="relative inline-block rounded-2xl bg-primary/10 px-6 py-2 text-[42px] text-primary neon-glow-text sm:text-7xl md:text-8xl lg:text-9xl">
            piloto
          </span>
        </span>
        <span className="mt-2 block text-[42px] sm:text-7xl md:text-8xl lg:text-9xl">
          automático.
        </span>
      </h1>

      {/* Subtítulo descriptivo - Fuente estándar del sistema */}
      <p className="relative z-10 mt-4 max-w-[280px] text-base font-medium leading-relaxed text-muted-foreground sm:max-w-md md:text-xl lg:max-w-2xl">
      ...
      <div className="relative z-10 mt-6 flex flex-col items-center justify-center gap-8 md:flex-row md:gap-10">
        <Button size="lg" className="w-full sm:w-auto px-10 py-7 text-xl font-bold transition-transform hover:scale-105">
          Empieza Gratis
        </Button>
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="flex items-center -space-x-2">
            {avatars.map((src, index) => (
              <div key={index} className="ghost-border h-12 w-12 overflow-hidden rounded-full border-2 border-background shadow-xl">
                <Image
                  src={src}
